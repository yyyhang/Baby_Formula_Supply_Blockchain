...
export async function callDeployedContract(web3: Web3, account: Account, abi: any, data: string, address: string, methodSignature: string, args?: any[]) {
    ...
    // console.log("deploying...");
    await a_contract.methods[methodSignature](...(args ?? [])).send({
        from: account.address,
        gasPrice: gasPrice,
